...
<template>
    <div class="wxchat-container" :style="{backgroundColor: wrapBg}">
        <div class="window" id="window-view-container" :style="{maxHeight: maxHeight + 'px', width: width +'px'}">
            ...
            <!-- main -->
            <ScrollLoader :minHeight="minHeight" @scroll-to-top="refresh" class="container-main" v-if="dataArray && dataArray.length>0" :style="{maxHeight: maxHeight-50 + 'px'}">
                <div class="message">
                    <ul>
...
<!--                                <div class="text" v-emotion="message.content" v-if="message.type==1"></div>-->
                                <div class="text" v-if="message.type==1"><span v-html="message.content"></span></div>
                                <!-- 图片 -->
        ...
        methods: {
            initData(){
              ...
              test(this.param).then(response => {
                me.dataArray = response.data.reverse();
              });
            },
